...
# Function to scan for connected devices and return vendor info
def scan_connected_devices():
    ...
    for sent, received in result:
        ...
        try:
            ...
            vendor = "Unknown"
        devices.append({'ip': received.psrc, 'mac': received.hwsrc, 'hostname': hostname, 'vendor': vendor})

    devices_info = "\n".join([
        f"{device['ip']}  {device['mac']}  {device['hostname']}  ({device['vendor']})"
...
class NetworkScannerApp(tk.Tk):
    ...
    def create_os_info_tab(self):
        ...
        self.os_button = tk.Button(self.os_tab, text="Detect OS of Gateway", command=self.refresh_os_info)
    ...
    def refresh_os_info(self):
        ...
        if ip.startswith("Error"):
            ...
            ip_parts = ip.split('.')
            ip_parts[-1] = '1'  # Assuming router IP is x.x.x.1
            gateway_ip = '.'.join(ip_parts)
            os_info = detect_os(gateway_ip)
        self.os_text.delete(1.0, tk.END)
        self.os_text.insert(tk.END, f"Detected OS of {gateway_ip}: {os_info}")
...
def main_scan():
    ...
    # Optional: If you want OS detection for each device found
    # for device in devices:
    #     print(f"\n[+] OS Detection for {device['ip']}:")
    #     os_info = detect_os(device['ip'])
    #     print(os_info)
